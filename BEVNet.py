...
class MFResBlock(nn.Module):
    ...
    def __init__(
        ...
        super(MFResBlock, self).__init__()
        if gate is None:
            self.gate = nn.ReLU(inplace=True)
        else:
            self.gate = gate
        ...
        self.conv1 = MFConv(in_channel=inplanes, out_channel=planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = norm_layer(planes)
        self.conv2 = MFConv(in_channel=planes, out_channel=planes, kernel_size=3, stride=1, padding=1, bias=False)
...
class MFCNN(nn.Module):
    def __init__(self, c1: int = 16, c2: int = 32, c3: int = 64, c4: int = 128, dim: int = 128,
        ...
        self.block1 = MFConvBlock(3, c1, self.gate, nn.BatchNorm2d)

        self.block2 = MFResBlock(inplanes=c1, planes=c2, stride=1,
...
class RECNN(nn.Module):
    def __init__(self, c1: int = 32, c2: int = 64, c3: int = 128, c4: int = 128, dim: int = 128, single_head: bool = True, ):
        super().__init__()
